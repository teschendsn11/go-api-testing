...
type ApiApiMonstersGetRequest struct {
	ctx context.Context
	ApiService *MonstersApiService
	challengeRating *[]float32
}

// The challenge rating or ratings to filter on.
func (r ApiApiMonstersGetRequest) ChallengeRating(challengeRating []float32) ApiApiMonstersGetRequest {
	r.challengeRating = &challengeRating
	return r
}

func (r ApiApiMonstersGetRequest) Execute() (*APIReferenceList, *http.Response, error) {
	return r.ApiService.ApiMonstersGetExecute(r)
}

/*
ApiMonstersGet Get list of monsters with optional filtering

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiApiMonstersGetRequest
*/
func (a *MonstersApiService) ApiMonstersGet(ctx context.Context) ApiApiMonstersGetRequest {
	return ApiApiMonstersGetRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *MonstersApiService) ApiMonstersGetExecute(r ApiApiMonstersGetRequest) (*APIReferenceList, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MonstersApiService.ApiMonstersGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/monsters"
	...
	if r.challengeRating != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("challenge_rating", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("challenge_rating", parameterToString(t, "multi"))
		}
	}
...
type ApiApiMonstersIndexGetRequest struct {
	ctx context.Context
	ApiService *MonstersApiService
	index string
}

func (r ApiApiMonstersIndexGetRequest) Execute() (*Monster, *http.Response, error) {
	return r.ApiService.ApiMonstersIndexGetExecute(r)
}

/*
ApiMonstersIndexGet Get monster by index.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param index The `index` of the `Monster` to get. 
 @return ApiApiMonstersIndexGetRequest
*/
func (a *MonstersApiService) ApiMonstersIndexGet(ctx context.Context, index string) ApiApiMonstersIndexGetRequest {
	return ApiApiMonstersIndexGetRequest{
		ApiService: a,
		ctx: ctx,
		index: index,
...
// Execute executes the request
//  @return Monster
func (a *MonstersApiService) ApiMonstersIndexGetExecute(r ApiApiMonstersIndexGetRequest) (*Monster, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MonstersApiService.ApiMonstersIndexGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/monsters/{index}"
