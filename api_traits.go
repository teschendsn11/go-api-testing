...
// TraitsApiService TraitsApi service
type TraitsApiService service

type ApiApiTraitsIndexGetRequest struct {
	ctx context.Context
	ApiService *TraitsApiService
	index string
}

func (r ApiApiTraitsIndexGetRequest) Execute() (*Trait, *http.Response, error) {
	return r.ApiService.ApiTraitsIndexGetExecute(r)
}

/*
ApiTraitsIndexGet Get a trait by index.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param index The `index` of the `Trait` to get.
 @return ApiApiTraitsIndexGetRequest
*/
func (a *TraitsApiService) ApiTraitsIndexGet(ctx context.Context, index string) ApiApiTraitsIndexGetRequest {
	return ApiApiTraitsIndexGetRequest{
		ApiService: a,
		ctx: ctx,
		index: index,
...
//  @return Trait
func (a *TraitsApiService) ApiTraitsIndexGetExecute(r ApiApiTraitsIndexGetRequest) (*Trait, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *Trait
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "TraitsApiService.ApiTraitsIndexGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/traits/{index}"
