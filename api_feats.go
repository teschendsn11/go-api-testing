...
// FeatsApiService FeatsApi service
type FeatsApiService service

type ApiApiFeatsIndexGetRequest struct {
	ctx context.Context
	ApiService *FeatsApiService
	index string
}

func (r ApiApiFeatsIndexGetRequest) Execute() (*Feat, *http.Response, error) {
	return r.ApiService.ApiFeatsIndexGetExecute(r)
}

/*
ApiFeatsIndexGet Get a feat by index.
...
A feat is a boon a character can receive at level up instead of an ability score increase.
 ...
 @return ApiApiFeatsIndexGetRequest
*/
func (a *FeatsApiService) ApiFeatsIndexGet(ctx context.Context, index string) ApiApiFeatsIndexGetRequest {
	return ApiApiFeatsIndexGetRequest{
		ApiService: a,
		ctx: ctx,
		index: index,
...
//  @return Feat
func (a *FeatsApiService) ApiFeatsIndexGetExecute(r ApiApiFeatsIndexGetRequest) (*Feat, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *Feat
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FeatsApiService.ApiFeatsIndexGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/feats/{index}"
