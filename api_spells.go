...
// SpellsApiService SpellsApi service
type SpellsApiService service

type ApiApiSpellsGetRequest struct {
	ctx context.Context
	ApiService *SpellsApiService
	level *[]int32
	school *[]string
}

// The level or levels to filter on.
func (r ApiApiSpellsGetRequest) Level(level []int32) ApiApiSpellsGetRequest {
	r.level = &level
	return r
}

// The magic school or schools to filter on.
func (r ApiApiSpellsGetRequest) School(school []string) ApiApiSpellsGetRequest {
	r.school = &school
	return r
}

func (r ApiApiSpellsGetRequest) Execute() (*APIReferenceList, *http.Response, error) {
	return r.ApiService.ApiSpellsGetExecute(r)
}

/*
ApiSpellsGet Get list of spells with optional filtering.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiApiSpellsGetRequest
*/
func (a *SpellsApiService) ApiSpellsGet(ctx context.Context) ApiApiSpellsGetRequest {
	return ApiApiSpellsGetRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *SpellsApiService) ApiSpellsGetExecute(r ApiApiSpellsGetRequest) (*APIReferenceList, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SpellsApiService.ApiSpellsGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/spells"
	...
	if r.level != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("level", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("level", parameterToString(t, "multi"))
		}
	}
	if r.school != nil {
		t := *r.school
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("school", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("school", parameterToString(t, "multi"))
		}
	}
...
type ApiApiSpellsIndexGetRequest struct {
	ctx context.Context
	ApiService *SpellsApiService
	index string
}

func (r ApiApiSpellsIndexGetRequest) Execute() (*Spell, *http.Response, error) {
	return r.ApiService.ApiSpellsIndexGetExecute(r)
}

/*
ApiSpellsIndexGet Get a spell by index.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param index The `index` of the `Spell` to get.  Available values can be found in the [`ResourceList`](#get-/api/-endpoint-) for `spells`. 
 @return ApiApiSpellsIndexGetRequest
*/
func (a *SpellsApiService) ApiSpellsIndexGet(ctx context.Context, index string) ApiApiSpellsIndexGetRequest {
	return ApiApiSpellsIndexGetRequest{
		ApiService: a,
		ctx: ctx,
		index: index,
...
// Execute executes the request
//  @return Spell
func (a *SpellsApiService) ApiSpellsIndexGetExecute(r ApiApiSpellsIndexGetRequest) (*Spell, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *Spell
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SpellsApiService.ApiSpellsIndexGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/spells/{index}"
