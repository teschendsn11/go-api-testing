...
type ApiApiFeaturesIndexGetRequest struct {
	ctx context.Context
	ApiService *FeaturesApiService
	index string
}

func (r ApiApiFeaturesIndexGetRequest) Execute() (*Feature, *http.Response, error) {
	return r.ApiService.ApiFeaturesIndexGetExecute(r)
}

/*
ApiFeaturesIndexGet Get a feature by index.
...
Extra Attack, Unarmored Defense, and Spellcasting.
 ...
 @param index The `index` of the feature to get.  Available values can be found in the [`ResourceList`](#get-/api/-endpoint-) for `features`. 
 @return ApiApiFeaturesIndexGetRequest
*/
func (a *FeaturesApiService) ApiFeaturesIndexGet(ctx context.Context, index string) ApiApiFeaturesIndexGetRequest {
	return ApiApiFeaturesIndexGetRequest{
		ApiService: a,
		ctx: ctx,
		index: index,
...
//  @return Feature
func (a *FeaturesApiService) ApiFeaturesIndexGetExecute(r ApiApiFeaturesIndexGetRequest) (*Feature, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FeaturesApiService.ApiFeaturesIndexGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/features/{index}"
